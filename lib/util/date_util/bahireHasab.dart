...
class BahireHasab extends Equatable {
  ...
  BahireHasab({int year = -1}) {
    year < 0 ? year = EtDatetime.now().year : year = year;
  }
  ...
  Map<String, dynamic> get nenewe {
    ...
    yeeletTewsak.forEach((el) => {
          if (el['key'] ==
              weekdays[(weekdays.indexOf(meskerem1) + metkih - 1) % 7])
            dayTewsak = el['value']
        });
    ...
    if (month == 2) {
      // ጥቅምት
      monthName = 'የካቲት';
      String tikimt1 = weekdays[(weekdays.indexOf(meskerem1) + 2) % 7];
      String metkihElet =
          weekdays[(weekdays.indexOf(tikimt1) + metkih - 1) % 7];
      yeeletTewsak.forEach((el) => {
            if (el['key'] == weekdays[weekdays.indexOf(metkihElet)])
              dayTewsak = el['value']
          });
    }
