...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin, RouteAware {
  ...
  SingleChildScrollView buildHomePageLoaded({required HomePageData homePageData}) {
    //GROUPS CONCATENATE
    List<Group> groups = [];
      ...
      itemBuilder: (context, index) {
        if (groups[index].groupItems.length > 0 && groups[index].isVisible) {
          return HomeGroups(
            ...
            groupTitle: L10nUtil.translateLocale(groups[index].groupTitleText, context),
            groupSubTitle: groups[index].groupSubTitleText != null
                ? L10nUtil.translateLocale(groups[index].groupSubTitleText!, context)
                : null,
            groupHeaderImageUrl:
                groups[index].headerImageId != null ? AppApi.baseUrl + groups[index].headerImageId!.imageSmallPath : null,
