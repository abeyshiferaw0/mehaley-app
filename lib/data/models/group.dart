...
class Group extends Equatable {
  final int? groupId;
  final TextLan groupTitleTextId;
  final TextLan? groupSubTitleTextId;
  ...
  Group({
    ...
    required this.groupTitleTextId,
    required this.groupSubTitleTextId,
  ...
  factory Group.fromMap(Map<String, dynamic> map, bool isAutoGenerated) {
    //PARSE GROUP ITEMS BASED ON GROUP ITEM
    List<dynamic> parseGroupItems(
        GroupType mGroupType, List<dynamic> items, bool isAutoGenerated) {
      if (mGroupType == GroupType.SONG) {
        return items
            .map((song) => Song.fromMap(isAutoGenerated ? song : song['item']))
            .toList();
      } else if (mGroupType == GroupType.PLAYLIST) {
        return items
            .map((playlist) =>
                Playlist.fromMap(isAutoGenerated ? playlist : playlist['item']))
            .toList();
      } else if (mGroupType == GroupType.ARTIST) {
        return items
            .map((artist) =>
                Artist.fromMap(isAutoGenerated ? artist : artist['item']))
            .toList();
      } else if (mGroupType == GroupType.ALBUM) {
        return items
            .map((album) =>
                Album.fromMap(isAutoGenerated ? album : album['item']))
            .toList();
      } else {
    ...
    return new Group(
      ...
      groupTitleTextId: TextLan.fromMap(map['group_title_text_id']),
      groupSubTitleTextId: map['group_sub_title_text_id'] != null
  ...
  Map<String, dynamic> toMap() {
    ...
    return {
      ...
      'group_title_text_id': this.groupTitleTextId,
      'group_sub_title_text_id': this.groupSubTitleTextId,
